...
const profile = () => {
  ...
  const fetchUserProfilePicture = async (userId: string) => {
    try {
      const response = await makeAuthenticatedRequest(
        `http://10.71.143.44:8080/users/${userId}/get-profile-image`
      );
  ...
  const fetchUsername = async (userId: string) => {
    try {
      const response = await makeAuthenticatedRequest(
        `http://10.243.71.82:8080/users/${userId}/get-username`
      );
  ...
  const fetchListingCover = async (listingId: string) => {
    try {
      const response = await makeAuthenticatedRequest(
          `http://10.243.71.82:8080/api/listings/${listingId}/get-cover-photo`
      );
  ...
  useEffect(() => {
    const fetchMyProfile = async () => {
      ...
      try {
        ...
        const response = await makeAuthenticatedRequest(
          'http://10.243.71.82:8080/profiles/me'
        );
  ...
  const uploadProfileImage = async (imageAsset: any) => {
    ...
    try {
      ...
      const response = await fetch(`http://10.243.71.82:8080/profiles/${myProfile?.profileId}/update/profile-image`, {
        method: 'PATCH',
        headers: {
  ...
  const removeExistingImage = async () => {
    try {
      ...
      const response = await fetch(`http://10.243.71.82:8080/profiles/${myProfile?.profileId}/delete/profile-image`, { // tufts
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${accessToken}`
        }
