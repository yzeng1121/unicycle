...
export const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  const verifyTokenWithBackend = async (token: string): Promise<boolean> => {
    try {
      const response = await fetch('http://10.243.71.82:8080/users/me', {
        method: 'GET',
        headers: {
  ...
  const refreshAccessToken = async (): Promise<string | null> => {
    try {
      ...
      const response = await fetch('http://10.243.71.82:8080/auth/refresh', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const logout = async (): Promise<void> => {
    try {
      ...
      if (refreshToken) {
        ...
        await fetch('http://10.243.71.82:8080/auth/logout', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
